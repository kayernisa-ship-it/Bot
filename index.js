...
const pending = {}; // chatId -> old query
...
async function askGemini(text) {
  if (!GEMINI_API_KEY) {
    return { category: "unclear", query: text, reply: "Do you mean a movie, song, or APK?" };
  }
...
Rules:
If the message is only a title/name and can be movie or song, use unclear.
If user clearly says movie, song, apk, return that category.
Return JSON only. No markdown.

Format:
{"category":"movie|song|apk|unclear","query":"clean search text","reply":"short English reply"}
`;
...
async function searchSite(site, query) {
  try {
    ...
    $("a").each((_, el) => {
      ...
      if (titleLower.includes(queryLower) || queryLower.split(" ").some(w => titleLower.includes(w))) {
...
bot.onText(/\/start/, (msg) => {
  bot.sendMessage(
    msg.chat.id,
    "Hello! Send any movie, song, or APK name. I will ask if it is unclear."
  );
});
...
bot.on("message", async (msg) => {
  ...
  if (pending[chatId] && ["movie", "song", "apk"].includes(lower)) {
    const oldQuery = pending[chatId];
    delete pending[chatId];
    return handleSearch(chatId, lower, oldQuery);
  }
